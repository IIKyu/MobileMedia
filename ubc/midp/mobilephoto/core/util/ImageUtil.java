...
/**
 * @author trevor
 * This is a utility class. It performs conversions between Image objects and 
 * byte arrays, and Image metadata objects and byte arrays. Byte arrays are the
 * main format for storing data in RMS, and for sending data over the wire.
 */
public class ImageUtil {
	...
	/**
	 * This method reads an Image from an Input Stream and converts it from
	 * a standard image file format into a byte array, so that it can be transported
	 * over wireless protocols such as SMS
	 */
	public byte[] readImageAsByteArray(String imageFile) {
		...
		try {
			...
			InputStream is = (InputStream) this.getClass().getResourceAsStream(imageFile);
	...
	/**
	 * 
	 *Convert the byte array from a retrieved RecordStore record into the ImageInfo ((renamed ImageData) object
	 * Order of the string will look like this:
	 * <recordId>*<foreignRecordId>*<albumName>*<imageLabel>
	 * Depending on the optional features, additional fields may be:
	 *<phoneNum>
	 */
	public ImageData getImageInfoFromBytes(byte[] bytes) {
		...
		//Get recordID int value as String - everything before first delimeter
	 ...
	 *<phoneNum>
	 */
	public byte[] getBytesFromImageInfo(ImageData ii) {

		//Take each String and get the bytes from it, separating fields with a delimiter
		...
		//Convert the 'Foreign' Record ID field for the corresponding Image record store
